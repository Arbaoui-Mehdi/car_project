describe 'Expectation Matchers' do
  ...
  describe 'predicate matchers' do
      ...
      # Can use these when methods nd in "?", require no arguments,
      # and return false/true
      ...
      expect(product.visible?).to be true   # exactly the as the previous ex

    end

    it 'will match have_* to custom methods like has_*?' do
			# changes "have_" to "has_", add "?" to end, calls method on object
      # Can use these when methods start with "has_", end in "?"
      # and return true/false. Can have argument, but not required
  ...
  describe 'observation matchers' do
    ...
    it 'will match when events change object attributes' do
			# call the test before the block
    ...
    it 'will match events change any values' do
      ...
      x = 10
      expect { x += 3 }.to change {x}.from(10).to(13)
      expect { x += 2 }.to change {x}.by(2)
      expect { x += 2 }.to change {x}.by_at_least(1)
    ...
    it 'will match when error are raised' do
  ...
  describe 'composing matchers' do
    ...
    it 'will match using non-phrase aliases for matcher' do
      # These are built-in aliases that make
      # specs read better by using noun-based
      # phrases instead of verb-base phrases

      # valid but akwards example
      fruits = ['apple', 'banana', 'cherry']
      expect(fruits).to start_with(start_with('a')) &
                    include(match(/a.a.a/)) &
                    end_with(end_with('y'))

      # imporoved version of the previous example
